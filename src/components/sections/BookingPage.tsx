...
const BookingPage: React.FC = () => {
  // useParams returns string | undefined, so we handle the comparison safely
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  
  // Fix: Compare as strings to avoid type mismatch errors
  const mentor = mentors.find(m => String(m.id) === id);
  
  const [sessionType, setSessionType] = useState<'virtual' | 'physical'>('virtual');

  if (!mentor) {
    return <div className="pt-32 text-center text-white">Mentor not found</div>;
  }
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-3 pb-4 border-b border-slate-700 mb-4">
                  <img 
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = `https://placehold.co/100x100/1e293b/ffffff?text=${mentor.name[0]}`;
                    }}
                  />
                  <div>
                ...
                <div className="space-y-2 text-sm mb-6">
                  ...
                  <div className="flex justify-between font-semibold text-white pt-2 border-t border-slate-700 text-base">
                    <span>Total</span>
